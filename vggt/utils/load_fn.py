...
def pil_to_float_tensor(img: Image.Image) -> torch.Tensor:
    """
    Convert a PIL RGB image to a float tensor in [0, 1] without relying on NumPy.
    Returns shape [C, H, W].
    """
    if img.mode != "RGB":
        img = img.convert("RGB")
...
def load_and_preprocess_images_square(image_path_list, target_size=1024):
    ...
    for image_path in image_path_list:
        ...
        original_coords.append([x1, y1, x2, y2, width, height])
        ...
        # Convert to tensor
        img_tensor = pil_to_float_tensor(square_img)
...
def load_and_preprocess_images(image_path_list, mode="crop"):
    ...
    for image_path in image_path_list:
        ...
        img = pil_to_float_tensor(img)  # Convert to tensor (0, 1) without NumPy bridge
